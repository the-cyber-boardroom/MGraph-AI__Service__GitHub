...
class test_Routes__GitHub__Secrets__Repo(TestCase):
    ...
    def test__init__(self):                                                                     # Test auto-initialization
        with self.routes as _:
            assert type(_)                    is Routes__GitHub__Secrets__Repo
            assert base_classes(_)            == [Fast_API__Routes, Type_Safe, object]
            assert _.tag                      == TAG__ROUTES_GITHUB_SECRETS_REPO
    ...
    def test__routes_paths(self):                                                               # Test route paths constant
        assert ROUTES_PATHS__GITHUB_SECRETS_REPO == [ '/github-secrets-repo/list'   ,
                                                      '/github-secrets-repo/get'    ,
                                                      '/github-secrets-repo/create' ,
                                                      '/github-secrets-repo/update' ,
                                                      '/github-secrets-repo/delete' ]
        assert len(ROUTES_PATHS__GITHUB_SECRETS_REPO) == 5

    # ═══════════════════════════════════════════════════════════════════════════════
    # list Tests
    # ═══════════════════════════════════════════════════════════════════════════════

    def test__list__success(self):                                                              # Test list returns repository secrets
        request_data = Schema__GitHub__Data__Request__Secrets__List(owner = self.repo_owner ,
                                                                    repo  = self.repo_name  )
        request  = Schema__GitHub__Request__Secrets__List(encrypted_pat = self.encrypted_pat ,
        ...
        with self.routes as _:
            ...
            assert type(result)                        is Schema__GitHub__Response__Secrets__List
            ...
            for expected_name in TESTING__REPO__SECRETS__NAMES:
    ...
    def test__list__invalid_repo(self):                                                         # Test list with nonexistent repo
        request_data = Schema__GitHub__Data__Request__Secrets__List(owner = self.repo_owner       ,
    ...
    def test__get__success(self):                                                               # Test get returns secret metadata
        secret_name  = TESTING__REPO__SECRETS__NAMES[0]
        request_data = Schema__GitHub__Data__Request__Secret__Get(owner       = self.repo_owner ,
                                                                  repo        = self.repo_name  ,
                                                                  secret_name = secret_name     )
        request  = Schema__GitHub__Request__Secret__Get(encrypted_pat = self.encrypted_pat ,
                                                        request_data  = request_data       )
        response = Response()
        ...
        with self.routes as _:
            ...
            assert type(result)                        is Schema__GitHub__Response__Secret__Get
            assert result.response_context.success     is True
            assert result.response_context.status_code == Enum__HTTP__Status.OK_200
    ...
    def test__get__not_found(self):                                                             # Test get with nonexistent secret
        request_data = Schema__GitHub__Data__Request__Secret__Get(owner       = self.repo_owner          ,
                                                                  repo        = self.repo_name           ,
    ...
    def test__create__success(self):                                                            # Test create secret successfully
        secret_name     = 'TEST_CREATE_SECRET_TEMP'
        ...
        request_data = Schema__GitHub__Data__Request__Secret__Create(owner           = self.repo_owner  ,
                                                                     repo            = self.repo_name   ,
                                                                     secret_name     = secret_name      ,
                                                                     encrypted_value = encrypted_value  )
        request  = Schema__GitHub__Request__Secret__Create(encrypted_pat = self.encrypted_pat ,
                                                           request_data  = request_data       )
        response = Response()
        ...
        with self.routes as _:
            ...
            assert type(result)                        is Schema__GitHub__Response__Secret__Create
            assert result.response_context.success     is True
    ...
    def test__update__success(self):                                                            # Test update secret successfully
        secret_name     = TESTING__REPO__SECRETS__NAMES[0]
        new_value       = 'updated_secret_value_456'
        encrypted_value = self.service_encryption.encrypt(
            Schema__Encryption__Request(value=new_value, encryption_type=Enum__Encryption_Type.TEXT)).encrypted

        request_data = Schema__GitHub__Data__Request__Secret__Update(owner           = self.repo_owner  ,
                                                                     repo            = self.repo_name   ,
                                                                     secret_name     = secret_name      ,
                                                                     encrypted_value = encrypted_value  )
        request  = Schema__GitHub__Request__Secret__Update(encrypted_pat = self.encrypted_pat ,
                                                           request_data  = request_data       )
        response = Response()
        ...
        with self.routes as _:
            ...
            assert type(result)                        is Schema__GitHub__Response__Secret__Update
            assert result.response_context.success     is True
            assert result.response_context.status_code == Enum__HTTP__Status.OK_200
    ...
    def test__delete__success(self):                                                            # Test delete secret successfully
        # First create a secret to delete
        secret_name     = 'TEST_DELETE_SECRET_TEMP'
        secret_value    = 'value_to_delete'
        encrypted_value = self.service_encryption.encrypt(
            Schema__Encryption__Request(value=secret_value, encryption_type=Enum__Encryption_Type.TEXT)).encrypted

        create_request_data = Schema__GitHub__Data__Request__Secret__Create(owner           = self.repo_owner  ,
                                                                            repo            = self.repo_name   ,
                                                                            secret_name     = secret_name      ,
                                                                            encrypted_value = encrypted_value  )
        create_request  = Schema__GitHub__Request__Secret__Create(encrypted_pat = self.encrypted_pat    ,
                                                                  request_data  = create_request_data   )
        self.routes.create(create_request, Response())

        # Now delete it
        delete_request_data = Schema__GitHub__Data__Request__Secret__Delete(owner       = self.repo_owner ,
                                                                            repo        = self.repo_name  ,
                                                                            secret_name = secret_name     )
        delete_request = Schema__GitHub__Request__Secret__Delete(encrypted_pat = self.encrypted_pat   ,
        ...
        with self.routes as _:
            ...
            assert type(result)                        is Schema__GitHub__Response__Secret__Delete
            assert result.response_context.success     is True
            assert result.response_context.status_code == Enum__HTTP__Status.OK_200
            assert result.response_data.deleted        is True

    def test__delete__not_found(self):                                                          # Test delete nonexistent secret
        request_data = Schema__GitHub__Data__Request__Secret__Delete(owner       = self.repo_owner          ,
                                                                     repo        = self.repo_name           ,
                                                                     secret_name = 'NONEXISTENT_DELETE_XYZ' )
        request  = Schema__GitHub__Request__Secret__Delete(encrypted_pat = self.encrypted_pat ,
                                                           request_data  = request_data       )
        response = Response()
    ...
    def test__setup_routes(self):                                                               # Test setup_routes registers all routes
        routes = Routes__GitHub__Secrets__Repo(github_api_factory = self.github_api_factory ,
