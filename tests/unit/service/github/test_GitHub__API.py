...
class test_GitHub__API(TestCase):
    ...
    @classmethod
    def setUpClass(cls):
        cls.test_repo_owner = 'test-owner'
        cls.test_repo_name  = 'test-repo'
        cls.test_repo       = f'{cls.test_repo_owner}/{cls.test_repo_name}'

        # Setup surrogate
        cls.surrogate_context = GitHub__API__Surrogate__Test_Context().setup()
        cls.surrogate_context.add_repo(cls.test_repo_owner, cls.test_repo_name)
    ...
    def setUp(self):
        self.api_token  = self.surrogate_context.admin_pat()
        self.api_url    = 'https://api.github.com'                              # GitHub__API still uses this for URL building
        self.github_api = GitHub__API(api_token=self.api_token)
    ...
    # ==========
    # tests that need a surrogate implementation of commits

    def test_get__commits(self):                                               # Test GET request for repository commits
        pytest.skip('Needs surrogate support for this app call')
