...
class test_Routes__GitHub__Secrets__Env__QA(TestCase):                              # QA tests for /github-secrets-env/* routes
    ...
    def setUpClass(cls):
        ...
        result = cls.secrets_client.list_env_secrets(cls.repo_owner, cls.repo_name, cls.environment_name)
        if result.get('response_context', {}).get('status_code') == 404:
            import pytest
            pytest.skip(f"Environment '{cls.environment_name}' does not exist - create it first")
    ...
    def test__list__success(self):                                                  # POST /github-secrets-env/list - List env secrets
        with self.secrets_client as _:
            result = _.list_env_secrets(self.repo_owner, self.repo_name, self.environment_name)
            ...
            print(f"\n  ✓ Found {len(secrets)} environment secrets in '{self.environment_name}'")
            for secret in secrets[:5]:
                print(f"    - {secret.get('name')}")

    def test__list__invalid_environment(self):                                      # POST /github-secrets-env/list - Invalid environment returns 404
        with self.secrets_client as _:
            result = _.list_env_secrets(self.repo_owner, self.repo_name, 'nonexistent-env-xyz')
    ...
    def test__get__not_found(self):                                                 # POST /github-secrets-env/get - Nonexistent secret
        with self.secrets_client as _:
            result = _.get_env_secret(self.repo_owner, self.repo_name, self.environment_name, 'NONEXISTENT_SECRET_XYZ')
    ...
    def test__full_secret_lifecycle(self):                                          # Create -> Get -> Update -> Delete
        secret_name  = 'QA_ENV_TEST_SECRET_TEMP'
        secret_value = 'qa_env_test_value_123'

        with self.secrets_client as _:
            # Step 1: Create
            create_result = _.create_env_secret(self.repo_owner, self.repo_name, self.environment_name, secret_name, secret_value)
            ...
            print(f"\n  ✓ Created env secret: {secret_name}")

            # Step 2: Get
            get_result = _.get_env_secret(self.repo_owner, self.repo_name, self.environment_name, secret_name)
            ...
            print(f"  ✓ Retrieved env secret: {secret_name}")

            # Step 3: Update
            updated_value = 'qa_env_updated_value_456'
            update_result = _.update_env_secret(self.repo_owner, self.repo_name, self.environment_name, secret_name, updated_value)
            ...
            print(f"  ✓ Updated env secret: {secret_name}")

            # Step 4: Delete
            delete_result = _.delete_env_secret(self.repo_owner, self.repo_name, self.environment_name, secret_name)
            ...
            print(f"  ✓ Deleted env secret: {secret_name}")

            # Step 5: Verify deletion
            verify_result = _.get_env_secret(self.repo_owner, self.repo_name, self.environment_name, secret_name)
